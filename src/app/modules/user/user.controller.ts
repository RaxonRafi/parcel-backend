...
const createUser = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    sendResponse(res, {
      ...
      message: pending_delivery
        ? "Registered as pending delivery personnel, waiting for admin approval"
...
const submitNid = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    const decodedToken = req.user as JwtPayload;
    const nidDetails = await UserServices.submitNid(
      payload,
      decodedToken.userId
    );
    sendResponse(res, {
...
const approveDeliveryPersonnels = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
...
export const getAllPendingDeliveryPersonnels = catchAsync(
  async (req: Request, res: Response, next: NextFunction) => {
    const query = req.query;
    const result = await UserServices.getAllPendingDeliveryPersonnels(
      query as Record<string, string>
    );
    sendResponse(res, {
      success: true,
      statusCode: 200,
      message: "Pending Delivery personnels fetched successfully",
      data: result.data,
      meta: result.meta,
    });
  }
